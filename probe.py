...
def probe(x, block_num, layer: str, epoch, iteration):
    ...
    with open(probe_result_path, 'a', newline='') as f:
        ...
        # 소수 둘째 자리까지만 기록
        writer.writerow([epoch, iteration, block_num,
                        '{:.4f}'.format(top1_mean), '{:.4f}'.format(top1_std),
                        '{:.4f}'.format(top3_mean), '{:.4f}'.format(top3_std),
                        '{:.4f}'.format(top1_percent_mean), '{:.4f}'.format(top1_percent_std),
                        '{:.4f}'.format(median_mean), '{:.4f}'.format(median_std)])
    ...
    with open(top3_indices_path, 'a', newline='') as f:
        ...
        for i, indices in enumerate(top3_indices):
            for rank, (row, col) in enumerate(indices, 1):
                writer.writerow([epoch, iteration, block_num, i, rank, row, col])