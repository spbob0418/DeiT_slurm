...
class Mlp(nn.Module):
    def __init__(
        ...
        self.fc1 = Quantized_Linear(
                                ...
                                weight_grad_quantize_module=Quantizer(w_gbits, 'per_channel'),
                                act_grad_quantize_module=Quantizer(a_gbits, 'per_token'),
...
class lowbit_VisionTransformer(VisionTransformer):
    def __init__(self, abits, wbits, w_gbits, a_gbits,
        ...
        # self.reg_token = nn.Parameter(torch.ones(1, self.prefix_token_num, embed_dim))
        self.reg_token = nn.Parameter(torch.rand(1, self.prefix_token_num, embed_dim))

        ####################White Patch Prefix#########################
        # prefix_token_np = np.load("/home/shkim/QT_DeiT_small/reproduce/zz_prefix_patch_token_for_initialization/raw_background_patch_token.npy")
        ...
        # prefix_token_tensor = prefix_token_tensor[:, :4, :]
    ...
    def forward_features(self, x, epoch, iteration, device_id):
        ...
        cls_token = x[:, :1, :]  # 첫 번째 토큰이 CLS 토큰
        image_patch_token = x[:, 1:, :]  # 나머지 토큰이 이미지 패치 토큰

        # Fake positional embedding for prefix token (Vertex)
        vertex_indices = [1, 14, 183, 196]

        vertex_pos_embed = self.pos_embed[:, vertex_indices, :]  # Extract from pos_embed
        prefix_token = prefix_token + vertex_pos_embed.expand(B, -1, -1)  # Add positional embedding

        
        x = torch.cat((cls_token, prefix_token, image_patch_token), dim=1)
        ...
        # if prefix_mode == 'register': 
        #     x = torch.cat((x[:, :1], x[:, 1 + self.prefix_token_num:]), dim=1)
